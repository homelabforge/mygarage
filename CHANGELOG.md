...
### Fixed
- Drive-session speed, RPM and temperature figures reflect the whole drive, not just the part recorded within WiFi range of home. A WiCAN buffers readings while it cannot reach the broker and replays them later with their original timestamps, but a session was summarised once when it closed, so readings that arrived afterwards were never counted: a drive that peaked at 85 km/h could be recorded as 20. A reading that lands inside a closed session's window now updates that session, and `backend/tools/recompute_session_aggregates.py` repairs sessions summarised before the fix.
