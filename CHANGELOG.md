...
- The Sessions tab labels distance and the odometer pair in your own units again, instead of showing them as `(unknown unit)`. That marker was correct while the stored number's unit was genuinely unknowable; now that devices declare their odometer unit and the backend converts on write, the value is canonical kilometres and can be rendered like any other distance. Live-tab telemetry parameters are still device-native and keep the marker.
