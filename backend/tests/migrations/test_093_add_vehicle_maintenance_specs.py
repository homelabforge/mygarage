"""Tests for migration 093 — vehicle maintenance-spec columns.
...
def test_093_adds_maintenance_spec_columns(engine_for_migration):
    _dialect, engine, _url = engine_for_migration
    _make_vehicles_table(engine)

    _load("093_add_vehicle_maintenance_specs").upgrade(engine)
...
def test_093_is_idempotent(engine_for_migration):
    """Second run is a no-op — no raise, no duplicate columns."""
    _dialect, engine, _url = engine_for_migration
    _make_vehicles_table(engine)
    mod = _load("093_add_vehicle_maintenance_specs")
    mod.upgrade(engine)
    mod.upgrade(engine)
    cols = {c["name"] for c in inspect(engine).get_columns("vehicles")}
...
def test_093_missing_table_skips(engine_for_migration):
