...
for service records and service visits.
"""
...
async def service_record(client: AsyncClient, auth_headers, test_vehicle):
    """Create a service record for attachment tests."""
    payload = {
        ...
        "cost": 75.00,
        "description": "Attachment test service record",
    }
...
@pytest.mark.asyncio
class TestServiceRecordAttachments:
    """Test service record attachment endpoints."""

    async def test_list_attachments_empty(self, client: AsyncClient, auth_headers, service_record):
        """Test listing attachments for a service record with no attachments."""
    ...
    async def test_upload_pdf_attachment(self, client: AsyncClient, auth_headers, service_record):
        """Test uploading a PDF attachment to a service record."""
        file_data = create_test_file("receipt.pdf")
        ...
        assert response.status_code == 201
        data = response.json()
        assert data["record_type"] == "service"
    ...
    @requires_write_access
    async def test_upload_image_attachment(self, client: AsyncClient, auth_headers, service_record):
        """Test uploading an image attachment."""
        file_data = create_test_file("photo.png")
    ...
    async def test_list_attachments_after_upload(
        ...
        """Test listing attachments after uploading."""
    ...
    @requires_write_access
    async def test_download_attachment(self, client: AsyncClient, auth_headers, service_record):
        """Test downloading an attachment."""
        # Upload an attachment
        file_data = create_test_file("download-test.pdf")
        upload_response = await client.post(
            f"/api/service/{service_record['id']}/attachments",
            headers=auth_headers,
            files={"file": file_data},
    ...
    @requires_write_access
    async def test_view_attachment(self, client: AsyncClient, auth_headers, service_record):
        """Test viewing an attachment inline."""
        # Upload an attachment
        file_data = create_test_file("view-test.png")
        upload_response = await client.post(
            f"/api/service/{service_record['id']}/attachments",
            headers=auth_headers,
            files={"file": file_data},
        ...
        # View the attachment
        response = await client.get(
            f"/api/attachments/{attachment['id']}/view",
            headers=auth_headers,
        )
    ...
    @requires_write_access
    async def test_delete_attachment(self, client: AsyncClient, auth_headers, service_record):
        """Test deleting an attachment."""
        # Upload an attachment
        file_data = create_test_file("delete-test.pdf")
        upload_response = await client.post(
            f"/api/service/{service_record['id']}/attachments",
            headers=auth_headers,
            files={"file": file_data},
    ...
    async def test_upload_to_nonexistent_record(self, client: AsyncClient, auth_headers):
        """Test uploading attachment to non-existent service record."""
        file_data = create_test_file("test.pdf")

        response = await client.post(
            "/api/service/99999/attachments",
            headers=auth_headers,
            files={"file": file_data},
        )
    ...
    async def test_list_attachments_nonexistent_record(self, client: AsyncClient, auth_headers):
        """Test listing attachments for non-existent service record."""
        response = await client.get(
            "/api/service/99999/attachments",
            headers=auth_headers,
        )
    ...
    async def test_download_nonexistent_attachment(self, client: AsyncClient, auth_headers):
    ...
    async def test_delete_nonexistent_attachment(self, client: AsyncClient, auth_headers):
    ...
    async def test_attachment_unauthorized(self, client: AsyncClient, service_record):
        """Test that unauthenticated users cannot access attachments."""
        # List
        response = await client.get(f"/api/service/{service_record['id']}/attachments")
