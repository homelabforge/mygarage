...
class PDFReportGenerator:
    ...
    def generate_service_history_pdf(
        ...
        if service_records:
            ...
            # Table headers
            table_data = [
                ['Date', 'Mileage', 'Type', 'Description', 'Cost', 'Vendor']
            ]
            ...
            for record in service_records:
                ...
                table_data.append([
                    ...
                    record.get('vendor_name', 'N/A')[:20] if record.get('vendor_name') else 'N/A',
                ])

            # Add total row
            table_data.append([
                '', '', '', Paragraph('<b>Total:</b>', self.styles['Normal']),
                Paragraph(f'<b>{self._format_currency(total_cost)}</b>', self.styles['Normal']), ''
            ])

            # Create table
            table = Table(table_data, colWidths=[0.9*inch, 0.8*inch, 1*inch, 2*inch, 0.9*inch, 1.2*inch])
            ...
            story.append(Paragraph("No service records found for this period.", self.styles['Normal']))
    ...
    def generate_cost_summary_pdf(
        ...
        # Summary table
        table_data = [
            ['Category', 'Count', 'Total Cost', 'Average']
        ]

        # Check if vehicle is motorized
        is_motorized = vehicle_info.get('vehicle_type') not in ['Trailer']

        # Build categories list - exclude Fuel for non-motorized vehicles
        categories = [
            ('Service & Maintenance', cost_data.get('service_count', 0), cost_data.get('service_total', 0)),
        ]

        if is_motorized:
            categories.append(('Fuel', cost_data.get('fuel_count', 0), cost_data.get('fuel_total', 0)))

        categories.extend([
            ('Collisions & Repairs', cost_data.get('collision_count', 0), cost_data.get('collision_total', 0)),
            ('Upgrades', cost_data.get('upgrade_count', 0), cost_data.get('upgrade_total', 0)),
        ])
        ...
        table_data.append([
            ...
            '',
            Paragraph(f'<b>{self._format_currency(grand_total)}</b>', self.styles['Normal']),
            '',
        ])
    ...
    def generate_tax_deduction_pdf(
        ...
        if deductible_records:
            ...
            table_data.append([
                ...
                Paragraph(f'<b>{self._format_currency(total_deductible)}</b>', self.styles['Normal']),
            ])
            ...
            story.append(Paragraph("No deductible expenses found for this period.", self.styles['Normal']))
    ...
    def generate_analytics_pdf(
        ...
        cost_summary_data = [
            ['Metric', 'Value'],
            ['Total Cost', self._format_currency(Decimal(str(cost_analysis.get('total_cost', 0))))],
            ['Average Monthly Cost', self._format_currency(Decimal(str(cost_analysis.get('average_monthly_cost', 0))))],
        ...
        if cost_analysis.get('cost_per_mile'):
            cost_summary_data.append(['Cost Per Mile', self._format_currency(Decimal(str(cost_analysis['cost_per_mile'])))])
        ...
        # Rolling Averages
        if cost_analysis.get('rolling_avg_3m') or cost_analysis.get('rolling_avg_6m') or cost_analysis.get('rolling_avg_12m'):
            ...
            if cost_analysis.get('rolling_avg_3m'):
                rolling_data.append(['3-Month', self._format_currency(Decimal(str(cost_analysis['rolling_avg_3m'])))])
            if cost_analysis.get('rolling_avg_6m'):
                rolling_data.append(['6-Month', self._format_currency(Decimal(str(cost_analysis['rolling_avg_6m'])))])
            if cost_analysis.get('rolling_avg_12m'):
                rolling_data.append(['12-Month', self._format_currency(Decimal(str(cost_analysis['rolling_avg_12m'])))])
            ...
            rolling_table.setStyle(TableStyle([
        ...
        if cost_projection:
            ...
            projection_data = [
                ...
                ['Monthly Average', self._format_currency(Decimal(str(cost_projection.get('monthly_average', 0))))],
                ['Next 6 Months', self._format_currency(Decimal(str(cost_projection.get('six_month_projection', 0))))],
                ['Next 12 Months', self._format_currency(Decimal(str(cost_projection.get('twelve_month_projection', 0))))],
            ]
            ...
            if assumptions:
                story.append(Spacer(1, 0.1*inch))
                story.append(Paragraph(f"<i>Assumptions: {assumptions}</i>", self.styles['InfoText']))
        ...
        if monthly_breakdown:
            ...
            monthly_table = Table(monthly_data, colWidths=[1.2*inch, 0.8*inch, 1.5*inch, 1.5*inch, 1.5*inch])
            monthly_table.setStyle(TableStyle([
        ...
        if service_breakdown:
            ...
            service_table = Table(service_data, colWidths=[2.5*inch, 1.5*inch, 1*inch, 1.5*inch])
        ...
        if vendor_data and vendor_data.get('vendors'):
            ...
            if vendor_data.get('highest_spending_vendor'):
                vendor_summary.append(['Highest Spending Vendor', vendor_data['highest_spending_vendor']])
            ...
            vendor_table = Table(vendor_table_data, colWidths=[2.5*inch, 1.5*inch, 1*inch, 1.5*inch])
        ...
        if seasonal_data and seasonal_data.get('seasons'):
            ...
            # Summary
            seasonal_summary = [
                ['Annual Average', self._format_currency(Decimal(str(seasonal_data.get('annual_average', 0))))],
            ]
            if seasonal_data.get('highest_cost_season'):
                seasonal_summary.append(['Highest Cost Season', seasonal_data['highest_cost_season']])
            ...
            seasonal_summary_table.setStyle(TableStyle([
                ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'),
                ('FONTNAME', (1, 0), (-1, -1), 'Helvetica'),
                ('FONTSIZE', (0, 0), (-1, -1), 10),
                ('ALIGN', (0, 0), (0, -1), 'RIGHT'),
            ...
            seasonal_table = Table(seasonal_table_data, colWidths=[1.5*inch, 1.5*inch, 1.5*inch, 1*inch, 1*inch])
    ...
    def generate_fleet_analytics_pdf(self, fleet_data: Dict[str, Any]) -> BytesIO:
        ...
        fleet_summary_data = [
            ['Category', 'Amount'],
            ['Fleet Value', self._format_currency(Decimal(str(total_costs.get('total_fleet_value', 0))))],
            ['Total Maintenance', self._format_currency(Decimal(str(total_costs.get('total_maintenance', 0))))],
            ['Total Fuel', self._format_currency(Decimal(str(total_costs.get('total_fuel', 0))))],
            ['Total Insurance', self._format_currency(Decimal(str(total_costs.get('total_insurance', 0))))],
            ['Total Taxes', self._format_currency(Decimal(str(total_costs.get('total_taxes', 0))))],
        ]
        ...
        fleet_summary_data.append(['Total Operating Cost', self._format_currency(grand_total)])

        fleet_summary_table = Table(fleet_summary_data, colWidths=[3*inch, 3*inch])
        fleet_summary_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#3b82f6')),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ...
        if cost_by_vehicle:
            ...
            vehicle_table = Table(vehicle_data, colWidths=[2*inch, 1.25*inch, 1.25*inch, 1.25*inch, 1.25*inch])
        ...
        if monthly_trends:
            story.append(Paragraph("Monthly Spending Trends (Last 12 Months)", self.styles['CustomSubtitle']))
            story.append(Spacer(1, 0.1*inch))
