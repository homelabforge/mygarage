...
async def download_service_history_csv(
    ...
    # Write header
    writer.writerow([
        'Date', 'Mileage', 'Service Type', 'Description', 'Cost',
        'Vendor Name', 'Vendor Phone', 'Notes'
    ])
...
async def download_all_records_csv(
    ...
    collision_query = select(ServiceRecordModel).where(ServiceRecordModel.vin == vin).where(ServiceRecordModel.service_type == 'Collision')
    if year:
    ...
    upgrade_query = select(ServiceRecordModel).where(ServiceRecordModel.vin == vin).where(ServiceRecordModel.service_type == 'Upgrades')
    if year:
