...
class InsurancePDFParser:
    ...
    async def parse_progressive_pdf(
        ...
        try:
            with pdfplumber.open(BytesIO(pdf_bytes)) as pdf:
                ...
                policy_num = self._extract_pattern(
                    full_text, self.progressive_patterns["policy_number"]
                )
                if policy_num:
                ...
                premium = self._extract_pattern(
                    full_text, self.progressive_patterns["total_premium"]
                )
                if premium:
                ...
                deductible = self._extract_pattern(
                    full_text, self.progressive_patterns["deductible"]
                )
                if deductible:
                ...
                # Add parsing note
                result["notes"] = (
                    f"Auto-imported from Progressive PDF on {datetime.now().strftime('%Y-%m-%d')}"
                )
    ...
    def _extract_vehicle_specific_data(self, text: str, vin: str) -> dict[str, Any]:
        ...
        if match:
            ...
            vehicle_premium = self._extract_pattern(
                section, self.progressive_patterns["vehicle_premium"]
            )
            if vehicle_premium:
    ...
    def _determine_frequency(self, start_date: str, end_date: str) -> str:
        ...
        try:
            ...
            logger.debug(
                "Failed to determine frequency from dates %s to %s: %s",
                start_date,
                end_date,
                e,
            )
