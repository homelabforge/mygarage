...
On hashing choice: widget keys are generated by ``secrets.token_urlsafe(32)``
(256 bits of entropy) and are never user-chosen. SHA-256 is the correct
primitive for hashing such high-entropy tokens — it provides collision
resistance at a per-request cost of ~microseconds, and the key space is
unbrute-forceable by construction. Password-hashing algorithms (Argon2,
bcrypt, PBKDF2) exist to slow offline attacks against *low-entropy* user
passwords; applying them to random tokens adds ~100ms of CPU per request
for zero security benefit. This matches industry practice for API tokens
(GitHub PATs, AWS access keys, and similar).
"""
...
def hash_widget_key(plaintext: str) -> str:
    ...
    See module docstring: widget keys are 256-bit random tokens from
    ``secrets.token_urlsafe(32)``, not user-chosen passwords. SHA-256 is
    the correct primitive here; a slow KDF would add latency without any
    security benefit against a token space that is unbrute-forceable.
    """
    # codeql[py/weak-sensitive-data-hashing] - 256-bit random token, not a password; see module docstring.
...
def widget_key_func(request: Request) -> str:
    ...
    if api_key.startswith(WIDGET_KEY_PREFIX):
        # codeql[py/weak-sensitive-data-hashing] - 256-bit random token, not a password; only used as a rate-limit bucket ID.
