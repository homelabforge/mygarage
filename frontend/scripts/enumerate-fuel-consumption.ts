#!/usr/bin/env bun
/**
 * The fuel-economy and fuel-rate read sites: an enumerator, not a transcription.
 *
 * ★ WHY THIS EXISTS AS A COMMITTED TOOL, and why it is a second file rather
 * than a flag on `enumerate-binary-distance.ts`. Task 6 migrated DISTANCE and
 * its brief was narrower than the manifest, so 23 rows it was assigned went
 * uncovered; task 6b takes the consumption family that was left. The brief for
 * this task states its own scope as "4 files, 29 sites" and calls that number a
 * FLOOR, which it is: the two chart-data transforms in `Analytics.tsx` decide
 * exactly the same question with a raw converter and a bare
 * `system === 'metric'` instead of a formatter, and a migration that moved the
 * axis LABEL onto the resolved token while leaving the plotted VALUES on the
 * binary one would put L/100km numbers under an MPG axis. So the vocabulary
 * below is the family, not the four named APIs, and the count is PRINTED by a
 * program rather than written down in prose that goes stale.
 *
 * ★ AST, NOT `grep`, AND BOTH, WHICH IS THE POINT. A parser cannot see a
 * mention in a comment, and `VehicleStatisticsCard.tsx:248` names
 * `formatFuelRate/getFuelRateUnit` in one. But "the AST found fewer" is only
 * evidence if you can see what it dropped and why, so this prints the TEXT
 * floor beside the AST answer and classifies every line the two disagree on.
 * Half an inventory is worse than none: a reader shown only the smaller number
 * cannot tell a correct exclusion from a miss.
 *
 ...
 *
 * Usage:
 *   bun run scripts/enumerate-fuel-consumption.ts            # the enumeration
 *   bun run scripts/enumerate-fuel-consumption.ts --json     # machine-readable
...
/**
 * The fuel-economy and fuel-rate family, by declaring class.
 *
 * ★ STATED WITH ITS BOUNDARY, because that is what a scope is. These are the
 * APIs through which a canonical L/100km or L/hr reaches a reader without that
 * reader's own `units.consumption` or `units.volume` token being consulted:
 * the two formatters and two label helpers whose parameter IS a binary
 * `UnitSystem`, plus the raw converters a call site reaches for when it makes
 * the same decision itself.
 *
 * ★ WHY THE RAW CONVERTERS ARE IN. `Analytics.tsx` plots two series whose axis
 * label comes from a binary label helper and whose VALUES come from
 * `system === 'metric' ? raw : UnitConverter.l100kmToMpg(raw)` and
 * `... : UnitConverter.litersToGallons(raw)`. Label and value are one decision
 * spelled twice; migrating only the half that happens to be a `UnitFormatter`
 * call would leave a chart plotting litres under an MPG axis. Task 6's
 * vocabulary included `kmToMiles`/`milesToKm` for exactly this reason.
 *
 * `litersToGallons`/`gallonsToLiters` are volume converters rather than
 * consumption ones, and they are here because the only production call to
 * either is a fuel RATE. They are named rather than assumed: if a future run
...
const VOCABULARY: readonly { className: string; method: string }[] = [
  ...
  { className: 'UnitFormatter', method: 'getFuelEconomyUnit' },
  { className: 'UnitFormatter', method: 'formatFuelRate' },
  { className: 'UnitFormatter', method: 'getFuelRateUnit' },
  { className: 'UnitConverter', method: 'l100kmToMpg' },
  { className: 'UnitConverter', method: 'lPer100kmToMpg' },
  { className: 'UnitConverter', method: 'mpgToL100km' },
  { className: 'UnitConverter', method: 'litersToGallons' },
  { className: 'UnitConverter', method: 'gallonsToLiters' },
]

/**
 * Directories excluded from the universe, with the reason.
 *
 * `src/utils/` holds the DEFINITIONS: `units.ts` declares every name above and
 * `unitAdapters.ts`/`unitFormat.ts` are the replacement they migrate onto. A
 * definition is not a call site, and counting them would make the migration
 ...
 * `X['formatFuelRate']()` is the same decision spelled differently, so it is
 * matched too.
 *
 ...
 *
 * A name-only match would report a module-local `formatFuelRate` taking a
 * resolved `UnitSet` as an unmigrated site, and a tool that reports correct
...
function main(): void {
  ...
  console.log('\nSCOPE: the fuel-economy and fuel-rate family, CALLED (not merely named), in')
