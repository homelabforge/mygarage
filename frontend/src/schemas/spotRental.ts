...
const nightlyRateSchema = z.coerce
  .number({
    invalid_type_error: 'Nightly rate must be a number',
  })
...
const utilitySchema = z.coerce
  .number({
    invalid_type_error: 'Utility cost must be a number',
  })
  .min(0, 'Utility cost cannot be negative')
  .max(9999.99, 'Utility cost too large (max $9,999.99)')

// Optional versions (allow empty string or undefined)
const optionalNightlyRateSchema = z.preprocess(
  (val) => (val === '' || val === undefined ? undefined : val),
  nightlyRateSchema.optional()
)

const optionalLargeRateSchema = z.preprocess(
  (val) => (val === '' || val === undefined ? undefined : val),
  largeRateSchema.optional()
)

const optionalUtilitySchema = z.preprocess(
  (val) => (val === '' || val === undefined ? undefined : val),
  utilitySchema.optional()
)
